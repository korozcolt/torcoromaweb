    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#verifyStatusForm').on('submit', function (e) {
                e.preventDefault();
                const id = $('#id').val();
                //ajax request to route('pqr.status',id) with id as parameter and return the response in the div with id status_pqr
                $.ajax({
                    url: "{{ route('support.verify', ':id') }}".replace(':id', id),
                    type: 'GET',
                    dataType: 'json',
